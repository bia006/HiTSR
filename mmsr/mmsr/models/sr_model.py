...
class SRModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if self.cri_pix is None and self.cri_perceptual is None:
            raise ValueError('All three pixel, perceptual losses are None.')
    ...
    def setup_optimizers(self):
        ...
        self.iters = self.opt['train']['niter']
    ...
    def feed_data(self, data):
        ...
        self.img_in_4x = data['img_in_4x'].to(self.device) # LR/ gt_down
        self.img_in_2x = data['img_in_2x'].to(self.device) # LR/ gt_down
        self.img_ref_4x = data['img_ref_4x'].to(self.device) # LR/ gt_down
        self.img_ref_2x = data['img_ref_2x'].to(self.device) # LR/ gt_down

        self.img_in_t = [self.img_in_4x, self.img_in_2x, self.gt, self.gt]
        self.img_ref_t = [self.img_ref_4x, self.img_ref_2x, self.img_ref, self.img_ref]
        
        
    def optimize_parameters(self, step):
        self.optimizer_s.zero_grad()
        self.output = self.net_g(self.img_in_lq, self.img_ref_lq)
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if save_img:
                if self.opt['is_train']:
                    ...
                    # writer.add_image('SR images', visuals)    
                else:
